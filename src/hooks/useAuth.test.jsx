  ...
  describe('Restauration depuis localStorage', () => {
    test('should restore valid user from localStorage', () => {
      ...
      mockLocalStorage.getItem.mockImplementation((key) => {
        switch (key) {
          case 'token': return 'stored-token';
          case 'token_expiry': return validExpiry;
    ...
    test('should logout if token is expired', () => {
      ...
      mockLocalStorage.getItem.mockImplementation((key) => {
        switch (key) {
          ...
          case 'token_expiry': return expiredExpiry;
          case 'user': return JSON.stringify(mockUser);
          default: return null;
        }
      });
  ...
  describe('Cas limites', () => {
    test('should handle missing user data in localStorage', () => {
      mockLocalStorage.getItem.mockImplementation((key) => {
        switch (key) {
          ...
          case 'token_expiry': return (Date.now() + 1000000).toString();
          case 'user': return null; // Pas de données user
          default: return null;
        }
      });
    ...
    test('should handle invalid expiry format', () => {
      mockLocalStorage.getItem.mockImplementation((key) => {
        switch (key) {
          case 'token': return 'valid-token';
          case 'token_expiry': return 'invalid-number';
          case 'user': return JSON.stringify({ name: 'Test' });
          default: return null;
        }
      });
