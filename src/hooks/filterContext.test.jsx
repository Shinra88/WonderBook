...
function TestComponent() {
  ...
  return (
    <div>
      <div data-testid="selected-categories">Categories: {JSON.stringify(selectedCategories)}</div>
      <div data-testid="selected-year">Year: {selectedYear || 'empty'}</div>
      <div data-testid="selected-type">Type: {selectedType}</div>
      <div data-testid="min-year">MinYear: {minYear}</div>
      <div data-testid="current-year">CurrentYear: {currentYear}</div>
      <div data-testid="search-query">Search: {searchQuery || 'empty'}</div>
      <div data-testid="filter-read">FilterRead: {filterRead.toString()}</div>
      ...
      <button data-testid="set-year" onClick={() => setSelectedYear('2023')}>
        Set Year
      </button>
      <button data-testid="set-type" onClick={() => setSelectedType('et')}>
        Set Type
      </button>
      <button data-testid="set-min-year" onClick={() => setMinYear(2000)}>
        Set Min Year
      </button>
      <button data-testid="set-search" onClick={() => setSearchQuery('test search')}>
        Set Search
      </button>
      <button data-testid="set-filter-read" onClick={() => setFilterRead(true)}>
        Set Filter Read
      </button>
    </div>
  );
