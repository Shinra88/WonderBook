...
vi.mock('../../hooks/customHooks', () => ({
  useBestRatedBooks: () => ({
    bestRatedBooks: [
      { bookId: '1', title: 'Best Book 1', status: 'validated' }
    ],
    loading: false
  }),
  useLastAddedBooks: () => ({
    lastAddedBooks: [
      { bookId: '2', title: 'Last Book 1', status: 'validated' }
    ],
    loading: false
  }),
  useFilteredBooks: () => ({
...
vi.mock('../../components/Books/LastBook/LastBook', () => ({
  default: ({ lastAddedBooks }) => <div data-testid="last-book">{lastAddedBooks?.length || 0} last books</div>
}))
