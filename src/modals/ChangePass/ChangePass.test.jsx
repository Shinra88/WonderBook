  ...
  useTranslation: () => ({
    t: (key) => {
      const translations = {
        ...
        'ChangePass.PasswordRequirements': 'Password must contain at least 8 characters, uppercase, lowercase, number and special character',
...
describe('ChangePass Component', () => {
  ...
  test('should show password requirements error for invalid password', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Password must contain at least 8 characters, uppercase, lowercase, number and special character')).toBeInTheDocument()
    })
  })
  ...
  test('should not show errors for valid matching passwords', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByText('Password must contain at least 8 characters, uppercase, lowercase, number and special character')).not.toBeInTheDocument()
  ...
  test('should handle failed password change', async () => {
    ...
    await reactAct(async () => {
      fireEvent.change(screen.getByLabelText('Old Password'), { target: { value: 'wrongpassword' } })
      fireEvent.change(screen.getByLabelText('New Password'), { target: { value: 'ValidPassword123!' } })
      fireEvent.change(screen.getByLabelText('Confirm New Password'), { target: { value: 'ValidPassword123!' } })
    })
